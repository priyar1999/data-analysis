...
x1 = pd.DataFrame(x1, columns = 'Const S1 S2 R&D Administration Marketing'.split())

X_opt = x1.loc[:, ['Const', 'S1', 'S2', 'R&D', 'Administration', 'Marketing']]
...
X_opt = x1.loc[:, ['Const', 'S2', 'R&D', 'Administration', 'Marketing']]
regressor_OLS = sm.OLS(endog = y, exog = X_opt).fit()
regressor_OLS.summary()


X_opt = x1.loc[:, ['Const', 'R&D', 'Administration', 'Marketing']]
regressor_OLS = sm.OLS(endog = y, exog = X_opt).fit()
regressor_OLS.summary()

#This is the best - see adjusted R2
# Run all the tests on the final model to check if all auumptions hold true
X_opt = x1.loc[:, ['Const', 'R&D', 'Marketing']]
regressor_OLS = sm.OLS(endog = y, exog = X_opt).fit()
regressor_OLS.summary()

X_opt = x1.loc[:, ['Const', 'R&D']]
regressor_OLS = sm.OLS(endog = y, exog = X_opt).fit()
regressor_OLS.summary()
